...
namespace r5rs
{
  ...
  public:
    ...
    {
      return std::get<Error>(value).message;
    }
    ...
    requires regular_invocable_rc<Try, decltype(f), T>
  {
    ...
    if (std::holds_alternative<Error>(t.value)) { return result_t(std::move(std::get<Error>(t.value))); }
    return std::invoke(std::forward<decltype(f)>(f), std::get<T>(t.value));
  }

  template<value_concept T>
  inline Try<T> operator||(Try<T> lhs, Try<T> rhs)
  {
