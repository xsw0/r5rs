#ifndef R5RS_EXPRESSIONS_H
#define R5RS_EXPRESSIONS_H
...
namespace r5rs
{
  namespace expression
  {
    ...
    struct COD
    {
      using cod_t = std::variant<CODs *, Exp *, Definition *>;
      virtual ~COD() = default;
    ...
    struct Datum
    {
      using datum_t = std::variant<SimpleDatum *, ListDatum *, VectorDatum *>;
    ...
    {
      using def_t = std::variant<Define *, Definitions *>;
    ...
    struct Exp: COD
    {
      using exp_t = std::variant<Variable *, Literal *, Call *, Lambda *,
    ...
    public:
      ...
      explicit Formals(std::list<std::string> fixed = {},
    ...
    public:
      ...
      explicit Body(std::list<DefinitionPtr> defs, std::list<ExpPtr> exps)
    ...
    {
    public:
      using value_t = std::variant<char, bool, int64_t, std::string, Symbol>;
    ...
    {
    public:
      using value_t = std::variant<DatumPtr, bool, char, int64_t, std::string>;
    ...
    public:
      ...
      exp_t exp_type() override { return this; }
      explicit Call(ExpPtr op, std::list<ExpPtr> ops)
    ...
    public:
      ...
      explicit Lambda(std::shared_ptr<Formals> formals, std::shared_ptr<Body> body)
    ...
    public:
      ...
      explicit Conditional(ExpPtr test, ExpPtr consequent, ExpPtr alternate)
    ...
    public:
      ...
      explicit Assignment(std::string variable, ExpPtr exp)
    ...
    public:
      ...
      explicit Define(std::string var, ExpPtr exp)
        : variable(std::move(var)), exp(exp)
      {
      };
    ...
    template <typename T = nullptr_t> ParserPtr<Token, T> match(TokenType type)
    {
      return make_parser(
        make_function([=](IStream<Token> input) -> ParserResult<Token, T> {
          if (input.eof())
          {
