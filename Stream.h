...
namespace r5rs
{
  ...
  public:
    ...
    {
      if (cur + index < 0) { return Error{ "access negative position." }; }
      while (cur + index >= buffer->size())
      {
        Try<T> t;
        if (!func || !(t = std::invoke(*func))) { return Error{ "Eof" }; }
        buffer->push_back(*t);
      }
    ...
    IStream & operator+=(std::ptrdiff_t index) { cur += index; return *this; }
    IStream & operator-=(std::ptrdiff_t index) { cur -= index; return *this; }
    IStream operator+(std::ptrdiff_t index) const { return IStream(*this) += index; }
    IStream operator-(std::ptrdiff_t index) const { return IStream(*this) -= index; }
    ...
    {
      std::list<T> res;
      for (size_t i = 0; operator[](i); ++i) { res.push_back(*operator[](i)); }
      return res;
    }

    std::list<T> list()
    {
      return operator std::list<T>();
    }
  private:
