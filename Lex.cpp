...
{
  static auto func = make_function(
    [](char ch) {
      return Char{ ch };
    }
  );
...
{
  static auto func = make_function(
    [](IStream<char> stream) -> std::string {
      return stream.foldl(
        make_function(
...
{
  static auto func = make_function(
    [](std::list<char> stream) -> std::string {
...
Parser<Char, TokenType> r5rs::lex::single_symbol()
{
  static const std::unordered_map<char, TokenType> single{
    ...
    { '\'', TokenType::quote_symbol },
    { '`', TokenType::quasiquote_symbol },
    { ',', TokenType::unquote_symbol },
    { '.', TokenType::dot },
  };

  static auto find = make_function(
    [=](char ch) {
      auto it = single.find(ch);
      if (it == single.end()) { return TokenType::err; }
      return it->second;
    }
  );

  static auto cond = make_function(
    [](TokenType type) {
