...
{
  template<size_t n>
  void check_num(std::list<Reference> & args)
  {
    if (args.size() != n)
    {
  ...
  template<size_t I>
  Reference index(std::list<Reference> & args)
  {
    auto it = args.begin();
    std::advance(it, I);
    return *it;
  ...
  {
    check_num<1>(args);
    if (std::holds_alternative<nullptr_t>(*index<0>(args))) { return true; }
    return false;
  }
...
{
  if (body->exps.empty()) { throw std::runtime_error("empty body!"); }
